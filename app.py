...
with st.sidebar:
    st.markdown("### 📋 Sensor Data Selection")
    ...
    with m_col1:
        st.metric("TDS", f"{tds} ppm")
        st.metric("pH", f"{ph}")
        st.metric("Turbidity", f"{turbidity}")
    with m_col2:
        st.metric("Hardness", f"{hardness}")
        st.metric("Conduc.", f"{conductivity}")
    
    # Analyze button is now just for triggering a fresh fetch if needed
...
with col1:
    ...
    fig_bar = px.bar(params_df, x='Parameter', y='Value', color='Value', 
                     color_continuous_scale='Tealgrn', title="Reading Breakdown")
...
# --- LOGS SECTION ---
st.divider()
st.markdown("### 📜 Full System History")
